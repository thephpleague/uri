...
If you already have an instantiated `Query` object you can return all the query string deserialized arguments if any in an associative `array` similar to PHP's `parse_str` using the `Query::getParams` method:
...
public static function Query::parse(string $query): array
public static function Query::build(array $pairs): string
...
This class mainly represents the query string as a collection of key/value pairs. The library does not rely on PHP’s `parse_str` and `http_build_query` functions to preserve its data.
...
This method parse the query string into an associative `array` of key/pairs value. The method expects three (3) arguments:
...
public function Query::withoutPairs(string[] $keys): Query
~~~
