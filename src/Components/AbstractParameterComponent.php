 ...
 * @since   1.0.0
 */
class AbstractParameterComponent implements Interfaces\Parameters
{
    /*
    ...
    {
        if ($query instanceof Interfaces\Parameters) {
            return $this->mergeParameters($query);
        }

        if ($query instanceof Traversable || is_array($query)) {
            return $this->mergeParameters(static::createFromArray($query));
        }

        return $this->mergeParameters(static::createFromArray($this->validate($query)));
    }

    /**
     * Merge two Interfaces\Parameters objects
     *
     * @param Interfaces\Parameters $query
     *
     * @return static
     */
    protected function mergeParameters(Interfaces\Parameters $query)
    {
