...
abstract class Validation
{
    ...
    protected static function validateSegment($data, $delimiter)
    {
        if (is_null($data)) {
            ...
            return iterator_to_array($data, false); //we don't need the index
        } elseif (is_string($data) || (is_object($data)) && (method_exists($data, '__toString'))) {
            $data = (string) $data;
            $data = trim($data);
            if (isset($data[0]) && $delimiter == $data[0]) {
                $data = substr($data, 1);
            }
     ...
     *
     * @param mixed $data the data to insert
     * @param array $host a annray representation of a host component
     *
     * @return array
     *
    ...
    protected static function removeSegment(array $data, $value, $delimiter)
    {
        ...
        $data = array();
        $res = explode($delimiter, substr($segment, 0, $pos).substr($segment, $pos + strlen($part)));
     ...
     * @return array
     */
    protected function sanitizeComponents(array $components)
    {
